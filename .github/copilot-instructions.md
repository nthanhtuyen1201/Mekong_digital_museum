## Copilot / AI instructions — NLNganh-QL
...
**Backend (backend/app.py, ~1171 lines)**

- Flask app on port 5000; serves static frontend from `../frontend` and `/uploads` for media files
...
- CORS enabled for dev; multipart/form-data handling for image/video/audio uploads
...
- `detail.html` + `detail.js` — item detail page with image gallery, Leaflet map, related items, HTML5 video/audio players
- `list.html` + `list.js` — browse/filter all items by type and province
- `admin.html` + `admin.js` — CRUD interface (tab-based for artifacts/events/places, multipart upload form)
...
**Province/item folder naming:** Uses `remove_vietnamese_marks()` function (app.py line 58) to convert Vietnamese diacritical marks to ASCII equivalents. This ensures "Đồng Tháp" → "dong_thap" (NOT "ong_thap" which `secure_filename()` would produce). Applied to both province and item names in all upload paths.

**Province mapping:** `derive_province(doc)` function (app.py line 80) extracts province from `location`/`address`/`address_after` fields and maps old Mekong Delta provinces to 6 merged units. Auto-called on create/update if `province` missing. Maps:
...
- `GET /api/places`, `/api/events`, `/api/artifacts` — list all items
...
- [frontend/js/detail.js](frontend/js/detail.js) — image gallery with lightbox, Leaflet map integration, related items rendering, HTML5 video/audio player rendering (lines 147-216)
...
**Video/Audio rendering** (detail.js lines 147-216):
...
4. **Province derivation** auto-called on create/update. Maps old provinces to 6 units. Don't bypass.
